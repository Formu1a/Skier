(function () {
    ...
    const CHUNK_SIZE = {
        width: TAMX / 3.0 / PIXELS_PER_METER,
        height: TAMY / 2.0 / PIXELS_PER_METER,
    };
    const QUADRANTS_PER_CHUNK = { x: 1, y: 1 };
    const QUADRANT_SIZE = {
        width: CHUNK_SIZE.width / QUADRANTS_PER_CHUNK.x,
        height: CHUNK_SIZE.height / QUADRANTS_PER_CHUNK.y,
    };
    ...
    const MIN_OBSTACLE_SPAWN_DISTANCE =
        Math.min(CHUNK_SIZE.height, CHUNK_SIZE.width) / 3;
    ...
    const DOWN_DURATION = 1000;

    const FLAMING_BUSH_PROB = 0.1;

    const MIN_DOG_SPAWN_Y_DISTANCE = (TAMY * 2) / PIXELS_PER_METER;
    const DOG_SPEED = 10 * TICK_DURATION;
    const DOG_SPAWN_INTERVAL = { min: FPS * 5, max: FPS * 20 };
    const DOG_HITBOX = {
        top: 0,
        left: 0,
        width: 22,
    ...
    const YETI_SPAWN_MILESTONE = 3000;
    ...
    function getInterval(obData) {
        return Math.round(
            randomRange(
                obData.distributionInterval.min,
                obData.distributionInterval.max
            )
        );
    }
    ...
    function updatePanelDistance() {
        distanceTraveled = Math.round(
            skier.distanceTraveled * PIXELS_PER_METER
        );
        updatePanel();
    }
            ...
            for (let qx = 0; qx < QUADRANTS_PER_CHUNK.x; ++qx) {
                ...
                let spawnPos = {
                    x:
                        quadrantCenter.x +
                        randomRange(
                            -halfQuadrantSize.width,
                            halfQuadrantSize.width
                        ),
                    y:
                        quadrantCenter.y +
                        randomRange(
                            -halfQuadrantSize.height,
                            halfQuadrantSize.height
                        ),
                };

                if (
                    vecDistance(objPos, spawnPos) <= MIN_OBSTACLE_SPAWN_DISTANCE
                ) {
                    continue;
                }
                ...
                addObstacle(
                    new obstacleToSpawn.construct(spawnPos, obstacleToSpawn)
                );
            }
        }
    ...
    function getRelativePosition(obj, skier) {
        return {
            top:
                (obj.globalPos.y - skier.globalPos.y) * PIXELS_PER_METER +
                SKIER_LOCATION.top,
            left:
                (obj.globalPos.x - skier.globalPos.x) * PIXELS_PER_METER +
                SKIER_LOCATION.left,
        };
    }
    ...
    function Skier() {
        ...
        this.updateChunks = function () {
            ...
            if (
                this.curChunk.x != prevChunk.x ||
                this.curChunk.y != prevChunk.y
            ) {
                let begin = {
                    x: this.curChunk.x - 2,
                    y: this.curChunk.y,
